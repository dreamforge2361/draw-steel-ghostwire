...
| `gold-line/map-gold-line-interior.webp` | Gold Line (Deadhead) | 6472×958 | Dual Hammerhead interior still (CyberMaps stitch). Fallback if WebM will not play. |
| `gold-line/map-gold-line-interior-loop.webm` | same (animated BG) | 6472×958, VP9, 24 fps, 8 s | **Preferred Foundry** Level background. World inject: **Scenes → Deadhead → Gold Line**. |
| `gold-line/map-gold-line-roofs.webp` | same (overhead) | 6472×958 | Matching roofs still. |
| `gold-line/map-gold-line-roofs-loop.webm` | same (overhead, animated) | 6472×958, VP9, 24 fps, 8 s | **Preferred** roof tile. Surface occlusion — roofs on until a token is underneath. |
