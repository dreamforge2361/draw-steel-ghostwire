...
**v1 stamping:** prefer scale-band Actor templates (micro/small/medium drone; bike/car/truck/…) stamped from Item data on Deploy. Treasure Items in the `vehicles` pack are **intentional** (ownership / ¥ / mods). Named SKU Actors (Mule-Bot, Lane-Hopper, Star-Chopper, Bulldog) are optional Director placement for unowned machines — Deploy still uses the band. Conjured sprites/elementals/spirits remain **Actor-only** (no Item).
