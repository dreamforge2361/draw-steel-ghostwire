...
**Date:** 2026-09-20  
**Module:** **0.3.36**  
**Status:** **SHIPPED** — stills + VP9 loops + world Scene inject + Deadhead journal notes. *Pending Foundry-verify (scene load, loop, roofs overhead).*  
...
2. Level **Interior** background = interior loop (HEAD-fallback to still).
3. Locked overhead tile **Roofs** = roofs loop, elevation 10, `occlusion.mode` **2 (SURFACE)** — roofs on until a token is underneath.
4. Grid **208** px = 5 ft; canvas **6472 × 958** (~31.1 × 4.6 squares). No baked grid in the art.
5. Already-present flagged Scene is left alone (Director edits stick). Re-apply missing tiles/levels: `game.ghostwire.ensureGoldLineScene({ force: true })`.
