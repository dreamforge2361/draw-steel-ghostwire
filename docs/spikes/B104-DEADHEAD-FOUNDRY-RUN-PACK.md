...
| Gold Line (dual Hammerhead) | **SHIPPED 0.3.36 (B106) / duration hotfix 0.3.39.** World inject **Scenes → Deadhead → Gold Line**. Prefer `map-gold-line-interior-loop.mp4` then `.webm` then still (same for roofs). Surface occlusion. Grid 208 / 5 ft, 6472×958. Stills are a valid playable layout if video fails. Beat remap L1 tail → R3 cab in the SoR + **Gold Line — Map Notes** journal. CyberMaps stitch only — do not drop a generated train. |
