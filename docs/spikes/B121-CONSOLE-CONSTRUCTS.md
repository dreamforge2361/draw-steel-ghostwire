...
1. **Lock A.** Scene token = roster anchor only. Wire play = Wired Console. Meat actions against that token stay off unless an ability bridges. Peer Console visibility does **not** reveal, unhide, or pan-to a rival’s meat token.
...
3. **Same-scene Overlay / Jacked In visibility.** Overlay and Jacked In compilers on the viewed scene see each other’s compiled sprites/Agents in Constructs **without Scan**. Linked does not. Command / Decompile / pan stay owner + Director.

## v1 behavior

- List compiled `kind: "sprite"` / `kind: "agent"` actors whose token **or** compiler token is on the viewed scene.
- Viewer: owner (construct or compiler) + Director + Overlay/Jacked In peer compilers on the same scene.
- Show: name, archetype, Stamina current/max, hybrid band, compiler, optional face chip.
- Actions (owner / Director): pan-to-anchor-token; **Command** opens the existing Compile Sprite / Compile Agent sheet (does **not** compile a new construct); **Decompile** calls `decompileSprite` / `decompileAgent`.
- Peer rows are Console-only (no pan / Command / Decompile).
...
4. Command opens the Compile sheet; Decompile still removes token+Actor; sheet Compile still compiles. Overlay/Jacked In peer compilers see each other in Constructs without Scan; tokens stay meat-side.
