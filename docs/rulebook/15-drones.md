...
**0.3.98 (S8):** the drone line is **40 chassis** — E1 15 / E2 8 / E3 8 / E4 9. New this pass: **Kiln-Beetle** (E3 armored repair), **Static Crow** (E3 EW + designator — EW-tagged only; see 0.3.99 below), **Tide-Wraith** (E4 apex aquadrone), **Second Face** (E4 apex anthro). Every chassis SKU now carries a real ¥ and stock Draw Steel **Project** fields (goal 150 / 300 / 450 / 600 by Echelon) so fabricating a frame eats a Lifestyle project slot like any other Project. The **Drone Vendor** kiosk preset picks up new SKUs with no listing edit.
**0.3.99:** **Static Crow** drops the **Mark** tag — it now ships **EW, Jump-In-Capable, Wired**. The kite still paints a lane, but that is an artefact of the EW suite, not a scout designation, so it does **not** grant **Street Eye** (`23-machines.md`). The Street Eye gate is unchanged: a Personal or Light **Air** frame tagged **Recon** / **Mark** / **Decoy**. **Spotter** (E1) and **Phantom** (E4) still qualify.
