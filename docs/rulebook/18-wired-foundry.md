...
Catalog: `assets/tokens/wired/library.json`. Atlas files: `assets/tokens/wired/node-relay.webp`, `node-host.webp`, `node-segment.webp` (png source + webp; `"placeholder": false`). Eight device styles shipped **0.3.49**. Twelve Conglomerates Host skins: HAL…NYX shipped **0.3.54**; **AEQ** / **LAZ** rows in catalog (`node-host-aeq` / `node-host-laz`, `placeholder: true` until plates land). Brand marks: `assets/brands/megacorps/brand-{ticker}.{png,webp}`. **Endpoint** is optional v1.1.
