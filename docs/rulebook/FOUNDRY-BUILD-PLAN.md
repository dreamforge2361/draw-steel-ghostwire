...
- **B103** Bestiary + L≤4 summon portrait-art plumbing — **landed, art forthcoming** (no bump). Spike: `docs/spikes/B103-BESTIARY-HUMANOID-PORTRAITS.md`. `tools/apply-bestiary-portrait-art.mjs` + `assets/tokens/{bestiary,summons}/` (57 Actors). Follow-up attaches WebPs, runs the tool (Actor `img` + token), bumps one patch.
