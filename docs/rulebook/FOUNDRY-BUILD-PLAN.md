...
Spec: `docs/spikes/B112-SCENE-WIRE-AUTO-NODES.md`. GM **Auto-nodes from Scene** on the Wired Console reads named AmbientLights + wall doors on the **viewed** Scene, writes Track 1 nodes onto `wiredBoard` (`{Room} - Light Control` R1, `{Room} - Maglock Door N` R2), places hidden 0.25 tokens via `placeNode` (B108 level + B110 scale) next to the first light / each door, links Light Control ↔ maglocks in the same room. Room parse: **`{Room Name} - {rest…}`** (space-hyphen-space required; no first-word fallback; missing splitter → warn + skip). Maglock names use the same ` - ` after the room (`Rear Car Substation - Maglock Door 1`, not `Rear Car Substation Maglock Door 1`). `autoFrom` flags make re-run skip (or replace). Cameras and Wire→meat toggles out of scope. **B113** Light/Maglock art ships with this module (`AUTO_NODE_TOKEN_ART` → `assets/tokens/wired/node-*.webp`). Does not touch `gold-line-scene.mjs`. **SHIPPED 0.3.48.**
- [x] **B112** auto-nodes (module **0.3.48**) — **pending Foundry-verify**. Done when:
...
Spec: `docs/spikes/B113-LIGHT-MAGLOCK-TOKEN-ART.md`. Michael **YES** art: cyan lightbulb + power symbol (`node-light-control`) and sliding doors + padlock (`node-maglock`) under `assets/tokens/wired/` (1254² PNG originals + 1024² WebP). B112 `placeNode` stamps Actor `img` + `prototypeToken.texture.src` + placed token texture. Generic Track 1/2 templates stay for other nodes. **SHIPPED 0.3.48.**
- [x] **B113** token art (module **0.3.48**) — **pending Foundry-verify**. Done when:
  - [x] PNG + WebP at `assets/tokens/wired/node-light-control.*` and `node-maglock.*`
...
Spec: `docs/spikes/B115-NPC-WIRE-KIT.md`. Droppable Matrix Support feature **Wire Kit — Matrix Verbs** grants the nine hero Matrix Verb UUIDs onto an NPC. Console / Token HUD **Add Wire Kit** for selected NPC tokens. Heroes skipped (already have verbs). No bestiary default. **SHIPPED 0.3.48.**
- [x] **B115** Wire Kit (module **0.3.48**) — **pending Foundry-verify**. Done when:
...
Spec: `docs/spikes/B116-NODE-TOKEN-LIBRARY.md`. Michael will send more node token styles. **This PR:** drop-in folder + `assets/tokens/wired/library.json` + board `tokenStyle` field + path helper. Auto Light/Maglock keep B113 defaults. **Follow-up:** Wired Console style picker. Does not block 0.3.48.
- [x] Catalog + README under `assets/tokens/wired/` (module **0.3.48**)
- [ ] Console `<select>` when placing/editing a node
- [ ] Foundry-verify extra styles after Michael drops files
...
- **B112** Scene → Wire auto-nodes — **SHIPPED 0.3.48, pending Foundry-verify**. Spike: `docs/spikes/B112-SCENE-WIRE-AUTO-NODES.md`. Console GM button; Light Control + Maglock from named lights / wall doors; B113 art stamped on those tokens.
- **B113** Light Control / Maglock token art — **SHIPPED 0.3.48, pending Foundry-verify**. Spike: `docs/spikes/B113-LIGHT-MAGLOCK-TOKEN-ART.md`. Michael YES art: `assets/tokens/wired/node-light-control.webp` + `node-maglock.webp` (1254² PNG sources alongside). Generic Track 1/2 templates unchanged.
- **B116** Node token library — **catalog 0.3.48, picker follow-up**. Spike: `docs/spikes/B116-NODE-TOKEN-LIBRARY.md`. Drop-in `assets/tokens/wired/library.json`; Auto Light/Maglock stay B113 defaults.
- **B114** Node-map readability — **SHIPPED 0.3.48, pending Foundry-verify**. Spike: `docs/spikes/B114-NODE-MAP-READABILITY.md`. Force/cluster layout, collision, zoom/pan, dense pills.
- **B115** NPC Wire Kit — **SHIPPED 0.3.48, pending Foundry-verify**. Spike: `docs/spikes/B115-NPC-WIRE-KIT.md`. Droppable feature grants Matrix Verbs onto NPCs; Console / HUD stamp. No bestiary default.
- **B104** Deadhead Foundry run pack — **SoR + empty pack scaffold 0.3.33**; **Gold Line map notes + Scene 0.3.36 (B106)**; **cargo remap folded 0.3.40**; **Director journal 0.3.42**; **Beat 0 hangout Scene 0.3.43 (B107)**; **Wire ping + Technomancer Whiteout 0.3.45**; **auto-nodes + node-map + Wire Kit 0.3.48 (B112 / B113 / B114 / B115)**. Spikes: `docs/spikes/B104-DEADHEAD-FOUNDRY-RUN-PACK.md`, `docs/spikes/B107-DEADHEAD-HANGOUT.md`, `docs/spikes/B106-WIRED-CONSOLE-WIRE-PING.md`, `docs/spikes/B109-TECHNOMANCER-DECKLESS-PAYLOADS.md`, `docs/spikes/B112-SCENE-WIRE-AUTO-NODES.md`, `docs/spikes/B113-LIGHT-MAGLOCK-TOKEN-ART.md`, `docs/spikes/B114-NODE-MAP-READABILITY.md`, `docs/spikes/B115-NPC-WIRE-KIT.md`. Director SoR `docs/directors/runs/deadhead/DEADHEAD-GOLD-LINE.md` + sidecar `GOLD-LINE-CARGO-REMAP.md` (LOCKED). JournalEntry pack `runs` (label **Ghostwire Runs**, GM-only) with intra-pack folder **Deadhead**. Remaining: Mama Brief + ARG wafer as Gear items. Hangout Scene injected 0.3.43 (still plate, new worlds only); canyon **SKIPPED** — narrated; Mama reused; Gold Line is Michael’s live Scene. No Trace −1 class abilities. Cargo maglev — not a passenger train. Do not overwrite Gold Line walls/lights/tiles.
