...
- **B104** Deadhead Foundry run pack — **SoR + empty pack scaffold 0.3.33**; **Gold Line map notes + Scene 0.3.36 (B106)**; **cargo remap folded 0.3.40**. Spike: `docs/spikes/B104-DEADHEAD-FOUNDRY-RUN-PACK.md`. Director SoR `docs/directors/runs/deadhead/DEADHEAD-GOLD-LINE.md` + sidecar `GOLD-LINE-CARGO-REMAP.md` (LOCKED). JournalEntry pack `runs` (label **Ghostwire Runs**, GM-only) with intra-pack folder **Deadhead**. Map-pack journal + world Scene inject shipped; remaining content pass (Overview, Trace 1–12, Beats 0–5, Items, Opposition; Mama Brief + ARG wafer; hangout / canyon + Mama reuse) still to ship. No Trace −1 class abilities. Cargo maglev — not a passenger train.
- **B106** Deadhead Gold Line dual-Hammerhead map pack — **SHIPPED 0.3.36, loop hotfix 0.3.37, cargo fiction 0.3.40, pending Foundry-verify**. Spike: `docs/spikes/B106-GOLD-LINE-MAP-PACK.md`. CyberMaps Hammerhead stitch (no generated train). `assets/maps/battlemaps/gold-line/` stills + VP9 loops; `data/scenes/gold-line.json` + `scripts/gold-line-scene.mjs` world inject (interior loop background, roofs overhead, grid 208 / 5 ft); SoR + Runs journal beat remap L1 aft freight → R3 cab. **0.3.37:** FilePicker / ranged GET (no HEAD); Level `background.video` loop/autoplay; `ensureGoldLineScene({ force: true })` restamps roof `texture.src`. **0.3.40:** cargo remap folded; Director hides roofs tile when inside; walls/lights Michael manual (do not overwrite).
