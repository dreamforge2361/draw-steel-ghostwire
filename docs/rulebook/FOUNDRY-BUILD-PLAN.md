...
- **B42c/B42d/B42e** RAW Passes A–C — locks applied, license + language scrub, class number holes closed from masters/packs (0.1.58, pending review). **B42e1** applied all twelve Michael locks — `docs/raw/` now carries **zero** `[PASS-C NEEDS MICHAEL]` markers. Remaining: **B47 Foundry-sync**, to bring `src/packs/**` text up to post-Pass-A/B/C wording (DC saves, "tier", "+1 to a roll", the Technomancer chrome-erosion rename, and the Commander **Command Persona** mode string) — see `docs/spikes/B42-RAW-REVIEW-FLAGS.md`.
