...
- **B103** Bestiary + L≤4 summon portrait art — **SHIPPED 0.3.35, pending Foundry-verify**. Spike: `docs/spikes/B103-BESTIARY-HUMANOID-PORTRAITS.md`. 40 bestiary + 17 summon WebPs; `tools/apply-bestiary-portrait-art.mjs`; Actor `img` + `prototypeToken.texture.src` + LevelDB rebuilt. Michael approved shipping.
- **B104** Deadhead Foundry run pack — **SoR + empty pack scaffold 0.3.33**; **Gold Line map notes + Scene 0.3.36 (B106)**. Spike: `docs/spikes/B104-DEADHEAD-FOUNDRY-RUN-PACK.md`. Director SoR `docs/directors/runs/deadhead/DEADHEAD-GOLD-LINE.md`. JournalEntry pack `runs` (label **Ghostwire Runs**, GM-only) with intra-pack folder **Deadhead**. Map-pack journal + world Scene inject shipped; remaining content pass (Overview, Trace 1–12, Beats 0–5, Items, Opposition; Mama Brief + ARG wafer; hangout / canyon + Mama reuse) still to ship. No Trace −1 class abilities.
- **B106** Deadhead Gold Line dual-Hammerhead map pack — **SHIPPED 0.3.36, loop hotfix 0.3.37, duration + roof-lock hotfix 0.3.39, pending Foundry-verify**. Spike: `docs/spikes/B106-GOLD-LINE-MAP-PACK.md`. CyberMaps Hammerhead stitch (no generated train). `assets/maps/battlemaps/gold-line/` stills + VP9 webm + H.264 mp4 loops; `data/scenes/gold-line.json` + `scripts/gold-line-scene.mjs` world inject (stills default, roofs at 3232, 475, elev 1, occlusion FADE, grid 208 / 5 ft); SoR + Runs journal beat remap L1 tail → R3 cab. **0.3.37:** FilePicker / ranged GET (no HEAD); Level `background.video` loop/autoplay; `ensureGoldLineScene({ force: true })` restamps roof `texture.src`. **0.3.39:** stills default until video is proven; skip `currentTime` when duration is non-finite; force restamps Michael roof lock; FADE (mode 1, alpha 0) — clear under tokens.
