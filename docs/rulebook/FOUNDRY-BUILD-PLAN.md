...
### B121 AEQ Mandate + LAZ Extract NPCs (2026-09-20 / 0.3.78)
Spec: `docs/spikes/B121-AEQ-LAZ-NPCS.md`. Four corp-line Actors under Corp & Security → Aequitas Mandate / Lazarus Extract. Shared Michael plates (1254² PNG + 1024² WebP). Wire Kit on all four; start Disconnected. Faction flags `aeq` / `laz`. Token vision on. Math mirrored from Corp Security Officer / Ironclad Subcommander / Street Doc / Rival Commander.
- [x] **B121** AEQ/LAZ NPCs (module **0.3.78**) — **pending Foundry-verify**. Done when:
  - [x] Four Actors + two plates; `img` + `prototypeToken.texture.src` stamped
  - [x] Wire Kit + LE / medic kit; faction flags; Has Vision
  - [x] RAW / Director note + Ten Conglomerates cross-link; packs rebuilt
  - [ ] Foundry-verify: drag each from Bestiary, Connect via Wire Kit, fire LE / med abilities
...
Spec: `docs/spikes/B116-NODE-TOKEN-LIBRARY.md`. Eight Michael YES styles under `assets/tokens/wired/` (`library.json` + `NODE_TOKEN_LIBRARY`). Wired Console GM **Token art** select writes board `tokenStyle`. Auto-nodes: Light → `light-control`, Maglock → `maglock`, Cam lights / named cams → `cam-controls`. Manual place: Director picks. Generic Track 1/2 when `tokenStyle` is empty. **SHIPPED 0.3.49.** Twelve Conglomerates Host skins (`node-host-{ticker}`) **0.3.54** (Charter Ten plates) + **0.3.79** AEQ/LAZ plates (`placeholder: false`); generic `node-host` stays default.
...
- **B111** ARG Argent Exchange bestiary portraits — **SHIPPED 0.3.47, pending Foundry-verify**. Spike: `docs/spikes/B111-ARG-TOKEN-ART.md`. Three Actors (Enforcer / Lieutenant / Officer); `bestiary/arg/` PNG+WebP; faction flag `arg`; Deadhead opposition lines. No Gold Line scene rewrite.
- **B121** AEQ Mandate + LAZ Extract NPCs — **SHIPPED 0.3.78, pending Foundry-verify**. Spike: `docs/spikes/B121-AEQ-LAZ-NPCS.md`. Four Actors (Mandate Trooper / Sergeant, Extract Medic / Chief Medic); shared Michael plates under `bestiary/aeq/` + `bestiary/laz/`; Wire Kit on all four; faction flags `aeq` / `laz`. No Gold Line `{ force: true }`. No PDF.
...
- **B115** NPC Wire Kit — **SHIPPED 0.3.49, Connect interface 0.3.68, pending Foundry-verify**. Spike: `docs/spikes/B115-NPC-WIRE-KIT.md`. Droppable feature marks NPCs/drones Wire-capable (and is a Connect interface); Console / HUD stamp. No generic bestiary default; **0.3.78** AEQ/LAZ Actors ship with the kit (Michael). Verbs stay on the applet.
