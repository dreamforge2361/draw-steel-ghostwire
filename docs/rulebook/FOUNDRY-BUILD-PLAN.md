...
Spec: `docs/spikes/B102-ARMOR-WEAPON-ITEM-ART.md`. JSON sources `src/packs/gear/{armor,weapons}/**` remain SoR (LevelDB is compiled). Item `img` convention: `modules/draw-steel-ghostwire/assets/tokens/{armor,weapons}/<dsid>.webp` (shields under `armor/`). Apply tool `tools/apply-gear-token-art.mjs` maps `_dsid` files onto Gear Item `img` **and** matching embedded `type: treasure` Items on pregen Heroes, then rebuilds `packs/gear` + `packs/pregens`. Empty staging dirs shipped; **art binaries + version bump wait for the upload follow-up.** Pregens store full Item copies (not live UUID links) — the Equipment tab shows the embed. Kit dsid collisions (Wren Longshot) are skipped because only treasure is stamped. General gear is out of scope.
- [x] **B102** plumbing (no module bump) — **art forthcoming**. Done when:
  - [x] Spike lists 22 armor/shields + 49 weapons vs pack `_dsid`s + pregen update rule
  - [x] `assets/tokens/armor/` + `assets/tokens/weapons/` exist
  - [x] `node tools/apply-gear-token-art.mjs --list` matches the pack
  - [ ] WebPs attached → apply script → one patch bump → Foundry-verify Gear pack + pregen Equipment tab
...
- **B102** Armor + weapon item-art plumbing — **landed, art forthcoming** (no bump). Spike: `docs/spikes/B102-ARMOR-WEAPON-ITEM-ART.md`. `tools/apply-gear-token-art.mjs` + `assets/tokens/{armor,weapons}/`. Follow-up attaches WebPs, runs the tool (gear + pregens), bumps one patch.
