...
- **B106** Deadhead Gold Line dual-Hammerhead map pack — **SHIPPED 0.3.36, pending Foundry-verify**. Spike: `docs/spikes/B106-GOLD-LINE-MAP-PACK.md`. CyberMaps Hammerhead stitch (no generated train). `assets/maps/battlemaps/gold-line/` stills + VP9 loops; `data/scenes/gold-line.json` + `scripts/gold-line-scene.mjs` world inject (interior loop background, roofs Surface overhead, grid 208 / 5 ft); SoR + Runs journal beat remap L1 tail → R3 cab.
