...
Spec: `docs/spikes/B116-NODE-TOKEN-LIBRARY.md`. Eight Michael YES styles under `assets/tokens/wired/` (`library.json` + `NODE_TOKEN_LIBRARY`). Wired Console GM **Token art** select writes board `tokenStyle`. Auto-nodes: Light → `light-control`, Maglock → `maglock`, Cam lights / named cams → `cam-controls`. Manual place: Director picks. Generic Track 1/2 when `tokenStyle` is empty. **SHIPPED 0.3.49.**
...
Spec: `docs/spikes/B117-CONSOLE-MATRIX-VERBS.md`. **PLAYER UX lock:** Connect on the sheet; player opens the Wired node facing them; that node UI shows Scan / Ping / Navigate; the verb fires from their actor (Instinct/Logic, Hacking / Jacked In / Reader). Director Console still has board + roster + the same shared strip — do not make players depend on GM Console ownership. Off-sheet verbs leave `hero.defaultItems` and the NPC Wire Kit stamp. Programs stay on the sheet. **SHIPPED 0.3.52.** Gold Line `{ force: true }` untouched. PDF later.
- [x] **B117** node-facing Matrix Verbs (module **0.3.52**) — **pending Foundry-verify**. Done when:
  - [x] Spike + Foundry notes in `18-wired-foundry.md` + Wire chapter In Foundry aside
  - [x] Node panel verb strip (token / HUD / minimap / sheet redirect); Console strip shares `useConsoleVerb`
  - [x] Roll uses the Connected player's characteristics + existing Hacking / Jacked In / Reader edges
  - [x] Sheet cleanup: Scan / Navigate / Ping / Broadcast / Search / Read-Write off defaultItems and Wire Kit stamp
  - [ ] Foundry-verify: Connected runner opens a revealed node and fires Scan/Ping/Navigate without the GM Console; Disconnected buttons disabled; sheet no longer lists those six
...
- **B116** Node token library — **SHIPPED 0.3.49, pending Foundry-verify**. Spike: `docs/spikes/B116-NODE-TOKEN-LIBRARY.md`. Catalog `assets/tokens/wired/library.json` (8 device styles); Console Token art select; Auto Light/Maglock/Cam keep locked defaults.
...
- **B117** Matrix Verbs from the node the player faces — **SHIPPED 0.3.53, pending Foundry-verify**. Spike: `docs/spikes/B117-CONSOLE-MATRIX-VERBS.md`. Connect / Jack Out / Toggle on the sheet; Scan / Ping / Navigate on the node panel (Console shares the fire path); Wire Kit stamps the three sheet verbs. No Gold Line rewrite.
- **B104** Deadhead Foundry run pack — **SoR + empty pack scaffold 0.3.33**; **Gold Line map notes + Scene 0.3.36 (B106)**; **cargo remap folded 0.3.40**; **Director journal 0.3.42**; **Beat 0 hangout Scene 0.3.43 (B107)**; **Wire ping + Technomancer Whiteout 0.3.45**; **auto-nodes + node-map + Wire Kit + node library 0.3.49 (B112 / B113 / B114 / B115 / B116)**; **Wire Atlas 0.3.50**; **atlas token art 0.3.51**; **Gear SKUs 0.3.52**. Spikes: `docs/spikes/B104-DEADHEAD-FOUNDRY-RUN-PACK.md`, `docs/spikes/B107-DEADHEAD-HANGOUT.md`, `docs/spikes/B106-WIRED-CONSOLE-WIRE-PING.md`, `docs/spikes/B109-TECHNOMANCER-DECKLESS-PAYLOADS.md`, `docs/spikes/B112-SCENE-WIRE-AUTO-NODES.md`, `docs/spikes/B113-LIGHT-MAGLOCK-TOKEN-ART.md`, `docs/spikes/B114-NODE-MAP-READABILITY.md`, `docs/spikes/B115-NPC-WIRE-KIT.md`, `docs/spikes/B116-NODE-TOKEN-LIBRARY.md`, `docs/spikes/B116-WIRE-ATLAS.md`. Director SoR `docs/directors/runs/deadhead/DEADHEAD-GOLD-LINE.md` + sidecar `GOLD-LINE-CARGO-REMAP.md` (LOCKED). JournalEntry pack `runs` (label **Ghostwire Runs**, GM-only) with intra-pack folder **Deadhead**. **Gear SKUs shipped 0.3.52** (Mama’s Deadhead Brief + ARG Courier Capsule in **Ghostwire Gear → Plot & Run Hooks**; gallery art, not regenerated). Hangout Scene injected 0.3.43 (still plate, new worlds only); canyon **SKIPPED** — narrated; Mama reused; Gold Line is Michael’s live Scene. No Trace −1 class abilities. Cargo maglev — not a passenger train. Do not overwrite Gold Line walls/lights/tiles.
