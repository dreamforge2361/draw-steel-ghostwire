...
Spec: `docs/spikes/B113-LIGHT-MAGLOCK-TOKEN-ART.md`. Cyan lightbulb + power symbol (`node-light-control`) and sliding doors + padlock (`node-maglock`) under `assets/tokens/wired/` (PNG source + 1024² WebP). B112 `placeNode` stamps Actor `img` + `prototypeToken.texture.src` + placed token texture. Generic Track 1/2 templates stay for other nodes. **SHIPPED 0.3.48.**
...
- **B113** Light Control / Maglock token art — **SHIPPED 0.3.48, pending Foundry-verify**. Spike: `docs/spikes/B113-LIGHT-MAGLOCK-TOKEN-ART.md`. `assets/tokens/wired/node-light-control.webp` + `node-maglock.webp` (PNG sources alongside). Generic Track 1/2 templates unchanged.
