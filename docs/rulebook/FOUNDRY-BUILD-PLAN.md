...
Spec: `docs/spikes/B112-SCENE-WIRE-AUTO-NODES.md`. GM **Auto-nodes from Scene** on the Wired Console reads named AmbientLights + wall doors on the **viewed** Scene, writes Track 1 nodes onto `wiredBoard` (`{Room} Light Control` R1, `{Room} Maglock Door N` R2), places hidden 0.25 tokens via `placeNode` (B108 level + B110 scale) next to the first light / each door, links Light Control ↔ maglocks in the same room. Room parse: ` - ` split, else leading words until Light/Control/Cam/Work/…. `autoFrom` flags make re-run skip (or replace). Cameras and Wire→meat toggles out of scope. **B113** Light/Maglock art is a null hook (`AUTO_NODE_TOKEN_ART`). Does not touch `gold-line-scene.mjs`. **SHIPPED 0.3.48.**
