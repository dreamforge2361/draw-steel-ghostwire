...
### B103 Bestiary humanoid portrait-art plumbing (2026-09-19)
Spec: `docs/spikes/B103-BESTIARY-HUMANOID-PORTRAITS.md`. JSON sources `src/packs/bestiary/**` remain SoR (LevelDB is compiled). Actor path convention: `modules/draw-steel-ghostwire/assets/tokens/bestiary/<slug>.webp`. Apply tool `tools/apply-bestiary-portrait-art.mjs` maps pack-slug files onto Actor `img` **and** `prototypeToken.texture.src`, then rebuilds `packs/bestiary`. Empty staging dir shipped; **art binaries + version bump wait for the upload follow-up.** Scope is 33 L1–4 corp / gang / E1 rival humanoids + veil-cultist. Critters, wilds, wire ICE, undead monsters, Mama (L5), and L6 corp bosses are out of scope.
- [x] **B103** plumbing (no module bump) — **art forthcoming**. Done when:
  - [x] Spike lists 33 humanoid slugs vs pack JSON + Actor `img` / token update rule
  - [x] `assets/tokens/bestiary/` exists
  - [x] `node tools/apply-bestiary-portrait-art.mjs --list` matches the pack
  - [ ] WebPs attached → apply script → one patch bump → Foundry-verify sheet + Scene token
...
- **B103** Bestiary humanoid portrait-art plumbing — **landed, art forthcoming** (no bump). Spike: `docs/spikes/B103-BESTIARY-HUMANOID-PORTRAITS.md`. `tools/apply-bestiary-portrait-art.mjs` + `assets/tokens/bestiary/`. Follow-up attaches WebPs, runs the tool (Actor `img` + token), bumps one patch.
- **B104** Deadhead Foundry run pack — **SoR + empty pack scaffold 0.3.33**. Spike: `docs/spikes/B104-DEADHEAD-FOUNDRY-RUN-PACK.md`. Director SoR `docs/directors/runs/deadhead/DEADHEAD-GOLD-LINE.md`. New JournalEntry pack `runs` (label **Ghostwire Runs**, GM-only) with intra-pack folder **Deadhead**. Content pass (Overview, Trace 1–12, Beats 0–5, Items, Opposition, Foundry checklist; Mama Brief + ARG wafer; hangout / canyon / 5-car train scenes + Mama reuse) still to ship. No art this bump; no Trace −1 class abilities.
