...
- **B106** Deadhead Gold Line dual-Hammerhead map pack — **SHIPPED 0.3.36, loop hotfix 0.3.37, two-tile working setup 0.3.39, pending Foundry-verify**. Spike: `docs/spikes/B106-GOLD-LINE-MAP-PACK.md`. CyberMaps Hammerhead stitch (no generated train). `assets/maps/battlemaps/gold-line/` stills + VP9 webm + H.264 mp4 loops; `data/scenes/gold-line.json` + `scripts/gold-line-scene.mjs` world inject (empty Level background; **Interior (motion)** tile 0,0 + **Roofs (motion)** tile 3232, 475, elev 1, sort 100, occlusion NONE; prefer mp4; grid 208 / 5 ft). Director hides the roof tile when the crew goes inside.
