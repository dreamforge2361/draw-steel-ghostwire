...
### B117 Matrix Verbs from Wired Console (2026-09-20)
Spec: `docs/spikes/B117-CONSOLE-MATRIX-VERBS.md`. Sheet keeps Connect / Jack Out / Toggle. Console fires Scan / Ping / Navigate (selected Connected actor + selected node) through Draw Steel `AbilityModel#use`. Soft Trace on tier-1 Ping/Navigate; Scan does not auto-move Trace. Off-sheet verbs leave `hero.defaultItems` and the NPC Wire Kit stamp. Programs stay on the sheet. **SHIPPED 0.3.52.** Gold Line `{ force: true }` untouched. PDF later.
- [x] **B117** Console Matrix Verbs (module **0.3.52**) — **pending Foundry-verify**. Done when:
  - [x] Spike + Foundry notes in `18-wired-foundry.md` + Wire chapter In Foundry aside
  - [x] Console verb strip; disabled while Disconnected
  - [x] Roll uses selected actor characteristics + existing Hacking / Jacked In / Reader edges
  - [x] Sheet cleanup: Scan / Navigate / Ping / Broadcast / Search / Read-Write off defaultItems and Wire Kit stamp
  - [ ] Foundry-verify: Connected runner + selected node fires Scan/Ping/Navigate; Disconnected buttons disabled; sheet no longer lists those six
...
- **B117** Matrix Verbs from Wired Console — **SHIPPED 0.3.53, pending Foundry-verify**. Spike: `docs/spikes/B117-CONSOLE-MATRIX-VERBS.md`. Scan / Ping / Navigate on the Console; Connect / Jack Out / Toggle on the sheet; Wire Kit stamps the three sheet verbs. No Gold Line rewrite.
