...
- **B106** Deadhead Gold Line dual-Hammerhead map pack — **SHIPPED 0.3.36, loop hotfix 0.3.37, duration hotfix 0.3.39, pending Foundry-verify**. Spike: `docs/spikes/B106-GOLD-LINE-MAP-PACK.md`. CyberMaps Hammerhead stitch (no generated train). `assets/maps/battlemaps/gold-line/` stills + VP9 webm + H.264 mp4 loops; `data/scenes/gold-line.json` + `scripts/gold-line-scene.mjs` world inject (interior loop background, roofs Surface overhead, grid 208 / 5 ft); SoR + Runs journal beat remap L1 tail → R3 cab. **0.3.37:** FilePicker / ranged GET (no HEAD); Level `background.video` loop/autoplay; `ensureGoldLineScene({ force: true })` restamps roof `texture.src`. **0.3.39:** prefer `loop.mp4` → `loop.webm` → still; skip `currentTime` when duration is non-finite; force always restamps 6472×958. Stills are a valid playable layout if video fails.
