...
- **B49** Equipment → usable ability + SFX link (bug) - `docs/directors/equipment-use-abilities.md` (**backlog**).
- **B43** Veil Rituals as DS Projects (explore) — `docs/directors/veil-rituals-exploration.md`.
- **B44** Pregens from Dossiers PDF — `docs/directors/pregens-dossiers.md`. Spike: `docs/spikes/B44-PREGENS-DOSSIERS.md` — **built at 0.1.62, pending Foundry-verify**: packs `pregens` (8 Level 1 Heroes, portraits from the PDF) and `pregen-fiction` (8 origin stories + index), generated by `tools/pregens-to-actors.mjs` and `tools/pregen-stories-to-journals.mjs`. Roster and assumptions: `docs/masters/pregens/ROSTER.md`, `BUILD-NOTES.md`.
