...
Spec: `docs/spikes/B116-WIRE-ATLAS.md`. Locked brief: device tokens = **what**, atlas tokens = **where**. Three altitudes (district Relay/Host → facility Segment → room Device). Scan Reach = hops on the **current Scene graph**. Dig-down procedure. Catalog stubs at `assets/tokens/wired/` (`node-relay` / `node-host` / `node-segment`); **no generated art**. Device library (eight YES styles + picker) shipped **0.3.49** (PR **#34**). This patch is **0.3.50** (Mama **0.3.48**). Rulebook journal page regenerated from RAW. PDF redo later. Gold Line `{ force: true }` untouched.
- [x] **B116** Wire Atlas (module **0.3.50**) — **pending Foundry-verify**. Done when:
  ...
  - [x] `assets/tokens/wired/library.json` atlas stubs + README; no png/webp generated
  - [ ] Foundry-verify: Rulebook → The Wire → Wire Atlas / topology page; catalog paths exist; no Gold Line rewrite
...
- **B116** Wire Atlas topology — **SHIPPED 0.3.50 (docs + journal + catalog stubs), pending Foundry-verify**. Spike: `docs/spikes/B116-WIRE-ATLAS.md`. RAW + Rulebook journal. Catalog stubs `assets/tokens/wired/node-relay|host|segment` (no art). Mama **0.3.48**; Wired Gold Line **0.3.49** (PR **#34**). No Gold Line rewrite. PDF later.
