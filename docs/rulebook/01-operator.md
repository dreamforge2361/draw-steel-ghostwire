...
Body Integrity (chrome capacity) uses the same formula as every other class: `6 + Physique + Echelon` (or the leaner `4 + Physique + Echelon`, still under review per the Chrome Rules v1 draft) -- see `GHOSTWIRE-Chrome-Rules-v1.md` and `GHOSTWIRE-Chrome-Catalog-v1.md` for full install rules, Echelon/grade doctrine, and pricing.
