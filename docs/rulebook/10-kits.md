...
**Notes for review:**
- Kits are cyber-fantasy reskins of Draw Steel kit archetypes (illustrative bonus numbers; final tuning deferred)
- Damage bonus lines use **Draw Steel Power Roll result order** (low / middle / high) — not the old inverted Ghostwire “Outcome Tier” ladder
- Gear progression uses **Availability** and **echelon-appropriate** gear, not a separate gear-tier ladder
- Ownership / Economy bridge called out in master — keep lean until Economy chapter
- Operator Merc still needs an explicit two-kit stacking rule when this chapter locks
- Hacker Cyberdecks and Wrench platforms are related but not the same as this shared Kits list

---

*Compiled for review, 2026-09-16. Canon source: "The Kits chapter — loadout doctrines & fighting styles" in `master_rules_baseline.md` (added 2026-07-15, unchanged through the 2026-07-29 baseline). On any conflict with the quick-reference chapter page, this baseline text wins. Bonus-line numbers are Draw Steel-benchmark illustrative values on **DS Power Roll result bands**; final tuning is deferred to the numeric/gear pass.*

## The design problem it solves

Draw Steel's Kit system (its Chapter 6 martial-loadout layer) assumes an abstract Wealth score and hands the hero a fixed weapon-and-armor package as flavor, because Draw Steel gear is light. GHOSTWIRE deliberately built the opposite — tracked nuyen, gear gated by Availability (and echelon-appropriate lists), and mod-slot crafting (the Economy and Chrome chapters). A naive port would give away free gear and silently override the whole Economy.
...
| **Breacher** | Sword and Board | Medium weapon + ballistic shield; medium armor | Push/prone control, front-line; medium armor plus a raised shield is the Stamina |
| **Warframe** | Shining Armor | Medium weapon + shield; heavy armor | Taunt signature (Frame Lock); heavy armor plus a raised shield is the Stamina |
...
## Review checklist (Kits)

- [ ] What a Kit is / is not reads cleanly for players
- [ ] Kit list + role grouping good for v1
- [ ] Improvised-weapons fallback OK
- [ ] Bonus-line numbers: keep as provisional DS-benchmark (DS order), or blank until numeric pass?
- [ ] Resolve Merc dual-kit stacking against this chapter

**Next after Species + Kits approval:** Stage 3 shared core (characteristics, Power Rolls, combat) or Stage 4 Wire / Veil / Machines — your call.
