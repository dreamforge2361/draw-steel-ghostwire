# Ghostwire Core Rulebook — Chapter: The Medic

**Status:** Stage 2 draft — batch review  
**Source of record:** `docs/masters/GHOSTWIRE_MEDIC_DEVELOPMENT_MASTER.md` Part 1  
**Draw Steel spine:** Troubadour (Steel Compendium)  
**Prior:** Operator + Scout **approved** 2026-09-16; remaining classes drafted together for one review pass

**Notes for review:**
- Heroic resource: Reagents (persist across encounters; refill in downtime) — GW-specific
- Subclasses: Street-Doc / Corp-Medtech / Ripperdoc
- Large invented 9/11-cost + subclass ability tables still provisional in master
...
> No Power Roll. A massive gas cloud fills the area for **3 rounds**. Each enemy in the area takes the **enhanced** Toxin dose — **4 + Instinct** poison damage over 2 rounds (half each round, round up on the first tick) — and is **Weakened** and **Slowed** until the end of its next turn after the last tick. The area stays hazardous terrain for all 3 rounds: an enemy that enters it or starts its turn there takes the dose again. *(B42e: magnitude derived from this chapter's own enhanced Toxin compound, the profile Chemical Warfare already described in words.)*
...
## Stage 2 review checklist (The Medic)
