...
**Module now:** **0.3.101** on main (I2 / B95 Chargen Wizard — Foundry smoke pending; last **PASS** was G2 armor/gadget mods + G1 Kit street-band grants at 0.3.100).  
**Just shipped:** **I2 / B95 Chargen Wizard — Appendix B clickable on the Hero sheet (0.3.101)**; G2 armor / gadget mod families + Hexshot's Street medium bow (0.3.100); G1 Kit chargen street-band grants + Static Crow Mark drop (0.3.99); S8 vehicle / drone / mods build-out (0.3.98); S6 Voidmark Director-only lore filter (0.3.97); F9 Black Market sell (0.3.96); F8 Ritual Seal artifacts + F6 Director Pay / Spend Hero (0.3.95).  
**Next picks:** **G4 B49 skill-on-weapon rolls** → **G3** lang + style tokens → **S1** → **L1** (locked queue; I2 Chargen Wizard shipped 0.3.101) · DJ1 district journals · F2 ritual applet polish · S2 broader SR gear reskin (now unblocked).  
...
| G3 | lang + style tokens | **Suggested after the locked G4 pass** (then S1, then L1). N4 PDF reprint is done and no longer ahead of it in the queue. |
| G4 | **B49 skill-on-weapon rolls** | **LOCKED 2026-09-22 (Michael). Next pick** — Chargen Wizard **I2 / 0.3.101** has shipped, so wire the owning skill's **+2** into B49 equipment-use attack rolls (`scripts/equipment-use.mjs` / weapon-use templates), using the auto-apply path that mirrors the Hacking edge pattern. Mapping: `weapons/heavy/` → **Heavy Weapons** (`heavyWeapons`); `light-firearms/` + `longarms/` → **Firearms**; `bows/` → **Firearms**; thrown **Thermite Charge / Shaped Charge** → **Demolitions**; vehicle-mounted guns → **Gunnery**; `melee/` → **Melee** even when the Kit keyword says heavy. Skills already exist in the registry. Smoke: **Fire Workhorse** vs **Fire Chatterbox**, with each skill alone. Optional **B44b Street-band heavy firearm** remains a separate content item, not this code pass. |
