...
/* -------------------------------------------- 0.3.134 (A) — plain language on the sealed card */

/**
 * The per-Working plain-language block, keyed by the Working's printed title.
 *
 * A sealed card used to print only the family's generic one-liner ("The warded room and its
 * thresholds — Magnitude 1"), which tells a new player nothing about what a bane buys them or when
 * the thing stops working. The same six lines the Formula Item carries now ride the sealed card too.
 *
 * Loaded at `ready` from scripts/data/ritual-plain-text.json (generated by
 * tools/rituals-plain-text.mjs from tools/data/ritual-plain-text.json). Empty until then, and an
 * unknown Working simply gets no block — never a guess.
 */
let RITUAL_PLAIN_TEXT = {};

/** Inject the table. Used by `ready` and by tools/wave-03134-smoke.mjs, which has no fetch. */
export function setRitualPlainText(table) {
  RITUAL_PLAIN_TEXT = (table && typeof table === "object") ? (table.cards ?? table) : {};
  return Object.keys(RITUAL_PLAIN_TEXT).length;
}

/** The plain-language entry for one Working title, or null. */
export function ritualPlainText(workingName, table = RITUAL_PLAIN_TEXT) {
  const name = String(workingName ?? "").trim();
  if (!name) return null;
  const cards = table?.cards ?? table ?? {};
  // Formula Items are named "Formula: Ward the Room"; the Working itself is "Ward the Room".
  return cards[name] ?? cards[name.replace(/^Formula:\s*/i, "")] ?? null;
}
...
export function ritualEffectHtml(payload) {
  ...
  const plain = ritualPlainText(payload?.workingName);
  const plainHtml = plain
    ? `<div class="ghostwire-ritual-plain">`
      + `<p><strong>${esc(loc("Plain.Story"))}</strong> ${esc(plain.story)}</p>`
      + `<p><strong>${esc(loc("Plain.Rules"))}</strong> ${esc(plain.plain)}</p>`
      + `<p><strong>${esc(loc("Plain.Means"))}</strong></p><ul>`
      + `<li><strong>${esc(loc("Fields.Magnitude"))}</strong> — ${esc(plain.magnitude)}</li>`
      + `<li><strong>${esc(loc("Fields.Scope"))}</strong> — ${esc(plain.scope)}</li>`
      + `<li><strong>${esc(loc("Fields.Sealed"))}</strong> — ${esc(plain.seal)}</li>`
      + `<li><strong>${esc(loc("Fields.Expires"))}</strong> — ${esc(plain.expires)}</li>`
      + `</ul></div>`
    : "";
...
export function registerRitualSeal() {
  // 0.3.134 (A): the plain-language block a sealed card prints. Fetched once, exactly as sfx.mjs
  // fetches its map; a failure leaves the table empty and the card falls back to the family line.
  Hooks.once("ready", async () => {
    try {
      const table = await foundry.utils.fetchJsonWithTimeout(`modules/${MODULE_ID}/scripts/data/ritual-plain-text.json`);
      const count = setRitualPlainText(table);
      console.log(`${MODULE_ID} | ritual plain text: ${count} Working(s) loaded`);
    } catch (error) {
      console.warn(`${MODULE_ID} | ritual plain text unavailable; sealed cards keep the family line only`, error);
    }
  });
  ...
  if (module) {
    module.api = {
      ...
      artifactForFamily, sealSucceeded, ritualPlainText, setRitualPlainText,
    };
  }
