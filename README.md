...
- `0.3.39` — **Gold Line duration hotfix (B106):** shipped VP9 webms report `duration=N/A` and Foundry throws `Failed to set currentTime ... non-finite` (roof drew as a mis-scaled scrap). Prefer H.264 `*-loop.mp4` (finite duration) then webm then still; skip seek when duration is non-finite; `force` always restamps 6472×958. Stills are a valid playable layout if video fails. Spike: `docs/spikes/B106-GOLD-LINE-MAP-PACK.md`.
...
Deadhead playtest (B104, **0.3.33** / map pack B106, **0.3.36** / duration hotfix **0.3.39**): Director SoR `docs/directors/runs/deadhead/DEADHEAD-GOLD-LINE.md`. JournalEntry pack **Ghostwire Runs** (`src/packs/runs/` → `packs/runs`) with folder **Deadhead** (map notes journal shipped). Gold Line Scene inject: `scripts/gold-line-scene.mjs` + `data/scenes/gold-line.json` (prefer `*-loop.mp4`). Rebuild with `node tools/build-packs.mjs runs`. Remaining content pass (Overview / Trace / wafers / hangout / canyon) is not in this bump. Spikes: `docs/spikes/B104-DEADHEAD-FOUNDRY-RUN-PACK.md`, `docs/spikes/B106-GOLD-LINE-MAP-PACK.md`.
