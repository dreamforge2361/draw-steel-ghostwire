...
- `0.3.48` — **Wired Gold Line playtest (B112 / B113 / B114 / B115).** GM **Auto-nodes from Scene** builds `{Room} - Light Control` (T1 R1) and `{Room} - Maglock Door N` (T1 R2) from named lights + wall doors, places hidden 0.25 tokens on the current Level with **B113** Light Control / Maglock art (`assets/tokens/wired/node-*.webp`; generic Track 1/2 templates stay for other nodes), skip/replace idempotent. Wired Node Map gets force/cluster layout, label collision avoidance, zoom/pan, truncated pills. Droppable **Wire Kit — Matrix Verbs** stamps the nine hero verbs onto an NPC (Console / HUD one-click). No bestiary default. Node token library catalog (`library.json`, B116 picker later). Does not touch the Gold Line live-scene lock. Spikes: `docs/spikes/B112-SCENE-WIRE-AUTO-NODES.md`, `B113-LIGHT-MAGLOCK-TOKEN-ART.md`, `B114-NODE-MAP-READABILITY.md`, `B115-NPC-WIRE-KIT.md`, `B116-NODE-TOKEN-LIBRARY.md`.
