...
- `0.3.39` — **Gold Line lock (B106):** Level background = interior motion MP4 (`map-gold-line-interior-loop.mp4`, loop + autoplay). ONE roof Tile (`map-gold-line-roofs-loop.mp4`) at 0,0 elev 1, occlusion NONE. No interior motion tile. Prefer `*-loop.mp4`. Director hides the roof to go inside. Skip seek when duration is non-finite. Spike: `docs/spikes/B106-GOLD-LINE-MAP-PACK.md`.
...
Deadhead playtest (B104, **0.3.33** / map pack B106, **0.3.36** / lock **0.3.39**): Director SoR `docs/directors/runs/deadhead/DEADHEAD-GOLD-LINE.md`. JournalEntry pack **Ghostwire Runs** (`src/packs/runs/` → `packs/runs`) with folder **Deadhead** (map notes journal shipped). Gold Line Scene inject: `scripts/gold-line-scene.mjs` + `data/scenes/gold-line.json` (Level interior MP4 + one roof tile at 0,0). Rebuild with `node tools/build-packs.mjs runs`. Remaining content pass (Overview / Trace / wafers / hangout / canyon) is not in this bump. Spikes: `docs/spikes/B104-DEADHEAD-FOUNDRY-RUN-PACK.md`, `docs/spikes/B106-GOLD-LINE-MAP-PACK.md`.
