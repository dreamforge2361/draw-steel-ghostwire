...
- `0.3.65` — **Scene kiosk type presets + street consumable SKUs (B119).** Token controls › cash register opens a type picker (Food / Medical / Tools / Armor / Weapons / Drones / empty). Placing from a type names the Actor (e.g. Street Food Kiosk) and pre-fills `flags.draw-steel-ghostwire.inventory` from catalog UUIDs (price null = catalog ¥). Restock from preset on the Director shop. New Gear pack shelf **Street Consumables**: six food/drink/supplement SKUs and four chems with real doses (Kickwire, Clearline, Numb-Tap, Red Dust) — maneuver use-ability, Active Effects, temp Stamina; Red Dust also +1 Taint. Infinite stock still. No Gold Line `{ force: true }`, no PDF. Spike: `docs/spikes/B119-KIOSK-PRESETS-CONSUMABLES.md`.
