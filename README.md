...
- `0.3.39` — **Gold Line duration + roof-lock hotfix (B106):** shipped VP9 webms report `duration=N/A` and Foundry throws `Failed to set currentTime ... non-finite`. Default to stills until video is proven; keep H.264 `*-loop.mp4` as an opt-in (finite duration). Skip seek when duration is non-finite. Bake Michael’s roof place **x=3232, y=475, 6472×958, elev 1, locked, occlusion FADE (mode 1, alpha 0)** — clear under tokens (Tile registration is not 0,0). Spike: `docs/spikes/B106-GOLD-LINE-MAP-PACK.md`.
...
Deadhead playtest (B104, **0.3.33** / map pack B106, **0.3.36** / duration + roof-lock hotfix **0.3.39**): Director SoR `docs/directors/runs/deadhead/DEADHEAD-GOLD-LINE.md`. JournalEntry pack **Ghostwire Runs** (`src/packs/runs/` → `packs/runs`) with folder **Deadhead** (map notes journal shipped). Gold Line Scene inject: `scripts/gold-line-scene.mjs` + `data/scenes/gold-line.json` (stills default; roof lock 3232, 475, elev 1, FADE occlusion). Rebuild with `node tools/build-packs.mjs runs`. Remaining content pass (Overview / Trace / wafers / hangout / canyon) is not in this bump. Spikes: `docs/spikes/B104-DEADHEAD-FOUNDRY-RUN-PACK.md`, `docs/spikes/B106-GOLD-LINE-MAP-PACK.md`.
