...
- `0.3.62` — **Ping vs Read/Write doctrine in the Ping card and Wire rules.** Ping is the lightest “touch this system” Matrix Verb — a nudge, not a deep hack (flick lights; tap a maglock to see if it’s live; test whether a cam answers / one-frame glitch). It does **not** unlock a maglock for entry or kill a camera feed — those are **Read/Write** (change data/settings/state). Lang Story/Effect + RAW `21` + Hacker doctrine + Foundry notes. VOIDMARK index rebuilt. Not Console Wire ping/spoof (B106). Docs/lang only. No Gold Line `{ force: true }`, no PDF.
