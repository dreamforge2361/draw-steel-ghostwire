...
- `0.3.54` — **Ten Conglomerates art.** Megacorp Host skins (`assets/tokens/wired/node-host-{ticker}.{png,webp}`) in the Wired node library; generic **Host** stays default. Brand marks at `assets/brands/megacorps/brand-{ticker}.{png,webp}` with one lore journal per conglomerate (Ghostwire Lore → Ten Conglomerates). Nox’s trash freighter in **Ghostwire Vehicles → Air** (`assets/tokens/vehicles/nox-trash-freighter.webp`) for Deadhead board/bail. Does not touch Gold Line. No PDF redo. Tickers: `docs/rulebook/MEGACORP-TICKERS.md`.
