...
Deadhead playtest (B104, **0.3.33** / map pack B106, **0.3.36** / cargo **0.3.40** / live-scene lock **0.3.41**): Director SoR `docs/directors/runs/deadhead/DEADHEAD-GOLD-LINE.md`. JournalEntry pack **Ghostwire Runs** (`src/packs/runs/` → `packs/runs`) with folder **Deadhead**. Gold Line Scene inject: `scripts/gold-line-scene.mjs` + `data/scenes/gold-line.json` (new worlds only; existing `goldLineScene` never rewritten). Rebuild with `node tools/build-packs.mjs runs`. Remaining content pass (Overview / Trace / wafers / hangout / canyon) is not in this bump. Spikes: `docs/spikes/B104-DEADHEAD-FOUNDRY-RUN-PACK.md`, `docs/spikes/B106-GOLD-LINE-MAP-PACK.md`.
