...
- `0.3.39` — **Gold Line two-tile working setup (B106):** Level background video is broken — leave it empty. Two Tiles play duration-valid MP4s: **Interior (motion)** at 0,0 elev 0; **Roofs (motion)** at 3232, 475 elev 1 sort 100, occlusion NONE. Prefer `*-loop.mp4` over webm. Director hides the roof tile when the crew goes inside. Skip seek when duration is non-finite. Spike: `docs/spikes/B106-GOLD-LINE-MAP-PACK.md`.
...
Deadhead playtest (B104, **0.3.33** / map pack B106, **0.3.36** / two-tile setup **0.3.39**): Director SoR `docs/directors/runs/deadhead/DEADHEAD-GOLD-LINE.md`. JournalEntry pack **Ghostwire Runs** (`src/packs/runs/` → `packs/runs`) with folder **Deadhead** (map notes journal shipped). Gold Line Scene inject: `scripts/gold-line-scene.mjs` + `data/scenes/gold-line.json` (empty Level background; interior + roof MP4 tiles). Rebuild with `node tools/build-packs.mjs runs`. Remaining content pass (Overview / Trace / wafers / hangout / canyon) is not in this bump. Spikes: `docs/spikes/B104-DEADHEAD-FOUNDRY-RUN-PACK.md`, `docs/spikes/B106-GOLD-LINE-MAP-PACK.md`.
