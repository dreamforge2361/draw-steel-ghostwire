...
- `0.3.53` — **Matrix Verbs from Wired Console (B117).** Scan / Ping / Navigate fire from the Console (selected Connected actor + selected node → Draw Steel power roll; Instinct Scan/Navigate, Logic Ping; Hacking / Jacked In / Reader edges). Soft Trace +1 on tier-1 Ping/Navigate; Scan stays observation (no auto Trace). Connect / Jack Out / Toggle stay on the sheet; those six verbs leave hero `defaultItems` and the NPC Wire Kit stamp. Programs stay on the sheet. No Gold Line rewrite, no PDF, no AI art. Spike: `docs/spikes/B117-CONSOLE-MATRIX-VERBS.md`.
