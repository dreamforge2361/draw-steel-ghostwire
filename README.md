...
- `0.3.54` — **Ten Conglomerates art + Deadhead aerial recon.** Megacorp Host skins (`assets/tokens/wired/node-host-{ticker}.{png,webp}`) in the Wired node library; generic **Host** stays default. Brand marks at `assets/brands/megacorps/brand-{ticker}.{png,webp}` with one lore journal per conglomerate (Ghostwire Lore → Ten Conglomerates). Nox’s trash freighter in **Ghostwire Vehicles → Air** (`assets/tokens/vehicles/nox-trash-freighter.webp`) for Deadhead board/bail. **Gold Line aerial recon** handout in **Ghostwire Runs → Deadhead** (`assets/items/deadhead/gold-line-aerial-recon.webp`; image Journal + Plot gear Item). Does not touch the live Gold Line Scene. No PDF redo. Tickers: `docs/rulebook/MEGACORP-TICKERS.md`.
...
- `0.3.43` — **Deadhead Beat 0 hangout (B107):** ships `assets/maps/battlemaps/map-deadhead-hangout.webp` (CyberMaps *Shady Workshop* still, 1920×1080) and world-injects **Scenes → Deadhead → Deadhead Hangout — Shady Workshop** on **new** worlds only. Still background — no loop, no Levels, no Tiles. Existing `deadheadHangoutScene` worlds are **never rewritten**; `{ force: true }` is GM-opt-in only. Walls / lights / tokens stay Michael manual. Canyon plate is **SKIPPED** — narrate the approach. Spike: `docs/spikes/B107-DEADHEAD-HANGOUT.md`.
...
Deadhead playtest (B104, **0.3.33** / map pack B106, **0.3.36** / cargo **0.3.40** / live-scene lock **0.3.41** / Director journal **0.3.42** / Wire ping + Technomancer Whiteout **0.3.45** / **Gear SKUs 0.3.52** / **Nox trash freighter + aerial recon 0.3.54**): Director SoR `docs/directors/runs/deadhead/DEADHEAD-GOLD-LINE.md`. JournalEntry pack **Ghostwire Runs** (`src/packs/runs/` → `packs/runs`) with folder **Deadhead** (Director journal + Gold Line map notes + aerial recon handout). Generate the Director journal with `node tools/deadhead-to-journals.mjs`. Rebuild with `node tools/build-packs.mjs runs`. Gold Line Scene inject: `scripts/gold-line-scene.mjs` + `data/scenes/gold-line.json` (new worlds only; existing `goldLineScene` never rewritten). Beat 0 hangout Scene inject: `scripts/deadhead-hangout-scene.mjs` + `data/scenes/deadhead-hangout.json` (new worlds only; existing `deadheadHangoutScene` never rewritten). Canyon Scene is **SKIPPED** — narrate the approach. **Gear SKUs shipped 0.3.52** (Mama’s Deadhead Brief + ARG Courier Capsule). **Nox trash freighter shipped 0.3.54** (Vehicles → Air). **Aerial recon photo shipped 0.3.54** (Journal + Plot gear Item; not baked into the battlemap). Spikes: `docs/spikes/B104-DEADHEAD-FOUNDRY-RUN-PACK.md`, `docs/spikes/B106-GOLD-LINE-MAP-PACK.md`, `docs/spikes/B106-WIRED-CONSOLE-WIRE-PING.md`, `docs/spikes/B107-DEADHEAD-HANGOUT.md`, `docs/spikes/B109-TECHNOMANCER-DECKLESS-PAYLOADS.md`.
