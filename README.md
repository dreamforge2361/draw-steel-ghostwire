...
- `0.3.53` — **Matrix Verbs from the node the player faces (B117).** Connect / Jack Out / Toggle stay on the sheet. Scan / Ping / Navigate fire from the Wired node token/panel (same DS power roll as sheet abilities: Instinct Scan/Navigate, Logic Ping; Hacking / Jacked In / Reader edges). Director Console still has board + roster + a shared verb strip — players do not need the GM to click Console. Soft Trace +1 on tier-1 Ping/Navigate; Scan stays observation (no auto Trace). Those six verbs leave hero `defaultItems` and the NPC Wire Kit stamp. Programs stay on the sheet. No Gold Line rewrite, no PDF, no AI art. Spike: `docs/spikes/B117-CONSOLE-MATRIX-VERBS.md`.
