...
- `0.3.39` — **Gold Line lock (B106):** new worlds: Level background = interior motion MP4 + ONE roof tile at 0,0 elev 1. Existing `goldLineScene` worlds are **never rewritten** on ready (walls/lights/tiles/background stay). `{ force: true }` is GM-opt-in only. Prefer `*-loop.mp4`. Skip seek when duration is non-finite. Spike: `docs/spikes/B106-GOLD-LINE-MAP-PACK.md`.
